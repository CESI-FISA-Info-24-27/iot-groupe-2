...
Endpoints :
  ...
  /stream/thermal      → Palette thermique
  /stream/highcontrast → CLAHE
  /health              → {"status":"ok"}
  /filters             → Liste JSON des filtres
"""

import os, cv2, time, threading, socketserver, numpy as np, logging, requests
...
class MJPEGStreamHandler(BaseHTTPRequestHandler):
    """Sert le flux MJPEG filtré aux clients HTTP."""
...
class FaceDetector:
        ...
        for d in ["/usr/share/opencv/haarcascades/", "/usr/share/opencv4/haarcascades/"]:
            ...
            if os.path.exists(f):
                ...
                if not self.cascade.empty():
                    print(f"✅ Cascade chargée: {f}", flush=True)
                    return
        raise RuntimeError("haarcascade_frontalface_default.xml introuvable")
    ...
    def apply_filter(self, frame, name):
        ...
        if name == "nightvision":
            g = f.copy(); g[:,:,0]=0; g[:,:,2]=0; g[:,:,1]=cv2.add(g[:,:,1],40); return g
        if name == "thermal":
    ...
    def read_esp32(self):
        ...
        print("✅ Flux ESP32 connecté", flush=True)
        buf = bytearray()
        boundary = b"--frame"
        for chunk in resp.iter_content(chunk_size=16384):
            ...
            while True:
                i = buf.find(boundary)
                if i == -1:
                    if len(buf) > 65536: buf = buf[-16384:]
                    break
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                print(f"❌ {e} → reconnexion 3s…", flush=True)
                time.sleep(3)
